...
class annonceView extends Controller
{
    public function annonce($transport_types,$fourchette_poid_mins,$fourchette_poid_maxs,$fourchette_volume_mins,$fourchette_volume_maxs,$moyen_transports,$wilayas,$annonce){
        return $this->head().$this->navbar().$this->header($annonce).$this->contenu($transport_types,$fourchette_poid_mins,$fourchette_poid_maxs,$fourchette_volume_mins,$fourchette_volume_maxs,$moyen_transports,$wilayas,$annonce).$this->modals($transport_types,$fourchette_poid_mins,$fourchette_poid_maxs,$fourchette_volume_mins,$fourchette_volume_maxs,$moyen_transports,$wilayas,$annonce).$this->scripts($annonce).$this->footer();
    } 

    public function head()
    {
        if(session()->exists('admin_name') || session()->exists('super_admin_name')){
            return (new acceuilAdminView)->headSecondAdmin();
        }
        else{
            return (new acceuilView)->headSecondhome();
        }
    }
    ...
    {
        if(session()->exists('admin_name') || session()->exists('super_admin_name')){
            return (new acceuilAdminView)->navbarSecondAdmin();
        }
        else{
            return (new acceuilView)->navbarSecond();
        }
    }
    ...
    {
        if(session()->exists('admin_name') || session()->exists('super_admin_name')){
            return (new acceuilAdminView)->modals().(new acceuilAdminView)->scripts();
        }
        else{
            return (new acceuilView)->footer().(new acceuilView)->modal_login().(new acceuilView)->script_login();;
        }
    }
              ...
              <div class="post">
                ...
                <div class="down-content">
                  ...
                  <ul class="post-info">
                    ...
                    <li>'.Carbon::createFromFormat('Y-m-d H:i:s', $annonce->created_at)->format('H:i:s d-m-Y').'</li>
                    <li>'.$annonce->status.'</li>';
                    $views=$annonce->views+1;
                    $code=$code.'<li>'.$views.' vues</li>
                  </ul>
                  <p>'.$annonce->texte.'</p>
                            ...
                            <div class="post-info">';
                              foreach ($annonce->tarjet->users as $transporteur){
                                if(session('id') != $transporteur->id){
                                  ...
                                  if($transporteur->statut == "certifié" && $exist==false){
            ...
            $code=$code.'<div class="col-lg-12">
              ...
              foreach ($annonce->transactions as $transaction){
                if($transaction->transporteur_id == session('id')){
                    $code=$code.'<p style="display:none;">';
                    $exist=true;
                    $code=$code.'</p>';
                }
            }
    ...
    public function modals($transport_types,$fourchette_poid_mins,$fourchette_poid_maxs,$fourchette_volume_mins,$fourchette_volume_maxs,$moyen_transports,$wilayas,$annonce)
    {
        $code='<!-- Modal noter-->
    ...
    public function scripts($annonce)
    {
        $code='<script>
